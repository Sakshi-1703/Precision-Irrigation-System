...
int predict_irrigation(float moisturePercentage, float flowRate, float temperature, float threshold_values[]) {
    if (moisturePercentage < threshold_values[0] && flowRate < threshold_values[1]) {
        return 1;  // Toggle irrigation system
    } else if (moisturePercentage < threshold_values[0] && flowRate >= threshold_values[1] && temperature < threshold_values[2]) {
        return 1;  // Toggle irrigation system
    } else {
...
<head>
  ...
  )rawliteral" + chartJS + R"rawliteral(
</head>
<body>
  <div class="topnav">
...
void loop() {
  ...
  int prediction = predict_irrigation(moisturePercentage, flowRate, temperature, threshold_values);
